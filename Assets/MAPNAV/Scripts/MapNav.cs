...
public class MapNav : MonoBehaviour
{
	...
	IEnumerator Start()
	{
		...
		if (triDView)
		{
			// KF: Commented this out to enable PTZ with 3D
			//pinchToZoom = false;
	...
	IEnumerator MapPosition()
	{
		...
		url = "http://open.mapquestapi.com/staticmap/v4/getmap?key=" + key + "&size=" + mapSize[indexSize].ToString() + "," + mapSize[indexSize].ToString() + "&zoom=" + zoom + "&type=" + maptype[indexType] + "&center=" + fixLat + "," + fixLon + "&scalebar=false";
		tempLat = fixLat;
		tempLon = fixLon;
	...
	IEnumerator ReScale()
	{
		...
		else
		{
			...
			while (Mathf.Abs(mycam.orthographicSize - targetOrtoSize * 0.625f) > 0.01f)
			{
				...
				currentOrtoSize = Mathf.MoveTowards(currentOrtoSize, targetOrtoSize * 0.625f, 2.5f * 3276800 * Time.deltaTime / Mathf.Pow(2, zoom) / mapScale);
	...
	void OnDisable()
	{
		// reparent the camera (if you need to)
		// reposition and rotate to camera
		cam.SetParent(null);
		cam.transform.position = new Vector3(0f, 300f);
		cam.transform.rotation = Quaternion.AngleAxis(90f, Vector3.right);
	...
	void Update()
	{
		...
		// KF: Added DEBUG statement to show PTZ variables in default GUI
		//status = ("PTZ=" + pinchToZoom + ", ITC=" + Input.touchCount + "\nMapping=" + mapping);
		...
		if (ready && !mapping && buttons)
		{
			...
			else
			{
				...
				if (GUI.RepeatButton(new Rect(2 * screenX / 5, 0, screenX / 5, screenY / 12), "zoom +"))
				{
					...
					{
						currentOrtoSize = mycam.orthographicSize;
						currentOrtoSize = Mathf.MoveTowards(currentOrtoSize, 3 * targetOrtoSize / 8, (100 / mapScale) * 32768 * Time.deltaTime / Mathf.Pow(2, zoom));
						mycam.orthographicSize = currentOrtoSize;
